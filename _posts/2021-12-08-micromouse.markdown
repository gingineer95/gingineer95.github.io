---
layout: post
title:  Micromouse Predator
date:   2021-12-08 12:00:00 +0300
image:  mm/robo_iso1.jpg
...
My initial thought was to use the PCB as a chassis component, as many of the Micromouse robots do. However, the MacIver lab uses a camera system above their habitat to track the mice and robot as pictured below. And in order to detect, track, and determine the orientation of the robot, 3 red LEDs need to form an isosceles triangle on top of the robot. Given that, it was simple to rework the mechanical design so the PCB was the top-most component.

<p align="center">
  <img width="600" height="600" src="{{ site.baseurl }}/images/mm/habitat.png">
</p>

As with all PCB prototyping, this was a very iterative process, especially considering how many components needed to fit in such a small space. The dimensions of the PCB were determined by the footprint of the robot, which in turn was driven by the length of the motors. I was able to fit 35 components in a 46cm x 34cm space. See images below:

<p align="center">
  <img width="600" height="600" src="{{ site.baseurl }}/images/mm/pcb_v4.png">
</p>

When the boards arrived, I soldered all the individual pins myself using a microscope. After everything was soldered, I quickly realized I couldn’t program my microcontroller. Unsure exactly where the issue was, I did a continuity test and discovered I didn’t have common ground across all the components. To get my PCB and running as quickly as possible, I soldered jumper wires to all components that needed common ground. Once I did that, I was able to program the PIC and confirm that all my other connections were correct. I designed a new board with common ground for future use. 
...
Below are images of the finalized, differential drive robot:

<p align="center">
  <img width="600" height="600" src="{{ site.baseurl }}/images/mm/robo_iso1.jpg">
</p>

I started with a 4 wheel configuration, where each wheel was 4 mm wide like many of the Micromouse designs. But after some testing, I discovered that the flooring in the MacIver lab habitat has a much higher coefficient of friction than the flooring used in the Micromouse competitions. I decided to remove two wheels and switch the tire material to harder rubber o-rings which were skinner and therefore had smaller areas of contact to make turns easier. The thinner wheels also had the benefit of making the overall width of the robot smaller.

## Remote Control

Video of robot being given left and right DC commands. These are only feed-forward commands, feed-back control is needed for precise movement.

(insert video of remote controls)


## Next Steps

Robot is tracked using the lab’s camera system. Next step is to use a PID controller to set a destination location and using feedback control, move the robot to that destination. After that, path planning with obstacles. Also adding the IR sensors.